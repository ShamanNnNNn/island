...
public class Fox extends Animal {
    ...
    private static final double MIN_WEIGHT = 7.0;
    ...
    protected void liveCycle() {
        ...
        hungerLevel += 2 + random.nextInt(10);
        age++;

        System.out.println(name + ": голод=" + hungerLevel + ", возраст=" + age + " лет");

        if (age > 10 && random.nextDouble() < 0.3) {
        ...
        if (hungerLevel > 80) {
            ...
            if (age > 1 && random.nextDouble() < 0.2) {
                reproduce();
            }
        }
    ...
    public boolean canEat(Object food) {
        if (food instanceof Animal) {
            ...
            return !(animal instanceof Wolf) &&
                    ...
                    animal.isAlive() &&
                    animal.getWeight() < this.weight * 1.5;
        }
        return false;
    }
    ...
    public void reproduce() {
        Fox partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
        ...
        if (random.nextDouble() < reproductionChance) {
            ...
            System.out.println(name + " и " + partner.getName() + " принесли потомство: " + childrenCount + " лисят--------------------------------");
        }
    }

    private Fox findReproductionPartner() {
        ...
        for (int dx = -searchRadius; dx <= searchRadius; dx++) {
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Fox && animal != this && animal.isAlive()) {
                            Fox otherFox = (Fox) animal;

                            if (otherFox.getAge() >= 1 && otherFox.getHungerLevel() <= 70) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherFox.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Fox babyFox = createBabyFox();
            Launcher.addFoxToVisualMap(babyFox);
            babyFox.startLife();
            System.out.println("Родился новый лис: " + babyFox.getName());
        }
    }

    private Fox createBabyFox() {
        String[] names = {"Рыжий", "Хитрый", "Быстрый", "Пушистый", "Огненный", "Лесной", "Ночной", "Зоркий"};
        String babyName = names[random.nextInt(names.length)] + " от " + name;

        return new Fox(babyName, 2.0 + random.nextDouble() * 2, 0, island);
    }

    @Override
    public double calculateFoodRequired() {
        double baseFood = maxFoodRequired * (weight / 20.0);
        if (hungerLevel > 70) baseFood *= 1.2;
        if (age > 8) baseFood *= 0.8;
        return Math.min(baseFood, 3.0);
    }
    ...
    public String toString() {
        return String.format("Лиса '%s' (%d лет, %.1f кг) [голод: %d/100] в %s",
                name, age, weight, hungerLevel,
                currentLocation != null ? currentLocation.getName() : "неизвестно");
    }

    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #FF8C00; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}