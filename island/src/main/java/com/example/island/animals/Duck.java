...
public class Duck extends Animal{
    ...
    public void reproduce() {
        ...
        Duck partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Duck findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Duck && animal != this && animal.isAlive()) {
                            Duck otherDuck = (Duck) animal;

                            if (otherDuck.getAge() >= 3 && otherDuck.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherDuck.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Duck babyDuck = createBabyDuck();
            Launcher.addDuckToVisualMap(babyDuck);            babyDuck.startLife();
            System.out.println("Родился новый олень: " + babyDuck.getName());
        }
    }

    private Duck createBabyDuck() {
        ...
        return new Duck(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
