...
public class Eagle extends Animal{
    ...
    public boolean canEat(Object food) {
        if (food instanceof Animal) {
            ...
            return !(animal instanceof Eagle) && animal.isAlive();
        }
        return false;
    }
    ...
    public void reproduce() {
        Eagle partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Eagle findReproductionPartner() {
        ...
        for (int dx = -searchRadius; dx <= searchRadius; dx++) {
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Eagle && animal != this && animal.isAlive()) {
                            Eagle otherEagle = (Eagle) animal;

                            if (otherEagle.getAge() >= 2 && otherEagle.getHungerLevel() <= 70) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherEagle.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Eagle babyEagle = createBabyEagle();
            Launcher.addEagleToVisualMap(babyEagle);
            babyEagle.startLife();
            System.out.println("Родился новый волк: " + babyEagle.getName());
        }
    }

    private Eagle createBabyEagle() {
        String[] names = {"Бурый", "Сильный", "Косолапый"};
        String babyName = names[random.nextInt(names.length)] + " от " + name;

        return new Eagle(babyName, 10.0 + random.nextDouble() * 5, 0, island);
    }
    ...
    public String toString() {
        return String.format("орел '%s' (%d лет, %.1f кг) [голод: %d/100] в %s",
                name, age, weight, hungerLevel,
                currentLocation != null ? currentLocation.getName() : "неизвестно");
    }

    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #FFFFF0; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}
