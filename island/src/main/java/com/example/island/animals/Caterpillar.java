...
public class Caterpillar extends Animal{
    ...
    public void reproduce() {
        ...
        Caterpillar partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Caterpillar findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Caterpillar && animal != this && animal.isAlive()) {
                            Caterpillar otherCaterpillar = (Caterpillar) animal;

                            if (otherCaterpillar.getAge() >= 3 && otherCaterpillar.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherCaterpillar.getName() + " на расстоянии " +
                                        Math.max(Math.abs(dx), Math.abs(dy)) + " клеток");
                                return otherCaterpillar;
                            }
                        }
                    }
                }
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Caterpillar babyCaterpillar = createBabyCaterpillar();
            Launcher.addCaterpillarToVisualMap(babyCaterpillar);            babyCaterpillar.startLife();
            System.out.println("Родился новый олень: " + babyCaterpillar.getName());
        }
    }

    private Caterpillar createBabyCaterpillar() {
        ...
        return new Caterpillar(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
    ...
    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #00FFFF; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}
