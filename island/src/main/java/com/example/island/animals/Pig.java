...
public class Pig extends Animal{
    ...
    public Pig(String name, double weight, int age, Island island) {
    ...
    public Pig(String name, Island island) {
        this(name,
    ...
    public void reproduce() {
        ...
        Pig partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Pig findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Pig && animal != this && animal.isAlive()) {
                            Pig otherPig = (Pig) animal;

                            if (otherPig.getAge() >= 3 && otherPig.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherPig.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Pig babyPig = createBabyPig();
            Launcher.addPigToVisualMap(babyPig);
            babyPig.startLife();
            System.out.println("Родилась новый свинья: " + babyPig.getName());
        }
    }

    private Pig createBabyPig() {
        String[] names = {"Толстый", "Грязный", "Розовый"};
        String babyName = names[random.nextInt(names.length)] + " от " + name;

        return new Pig(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
    ...
    public static Pig createRandomPig(Island island) {
        String[] names = {"Толстый", "Грязный", "Розовый"};
        String name = names[new Random().nextInt(names.length)];
        return new Pig(name, island);
    }

    @Override
    public String toString() {
        return String.format("Свинья '%s' (%d лет, %.1f кг) [голод: %d/100] в %s",
                name, age, weight, hungerLevel,
                currentLocation != null ? currentLocation.getName() : "неизвестно");
    }

    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #FF00FF; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}
