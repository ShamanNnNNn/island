...
public class Buffalo extends Animal{
    ...
    public void reproduce() {
        ...
        Buffalo partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Buffalo findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Buffalo && animal != this && animal.isAlive()) {
                            Buffalo otherBuffalo = (Buffalo) animal;

                            if (otherBuffalo.getAge() >= 3 && otherBuffalo.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherBuffalo.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Buffalo babyBuffalo = createBabyBuffalo();
            Launcher.addBuffaloToVisualMap(babyBuffalo);            babyBuffalo.startLife();
            System.out.println("Родился новый олень: " + babyBuffalo.getName());
        }
    }

    private Buffalo createBabyBuffalo() {
        ...
        return new Buffalo(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
