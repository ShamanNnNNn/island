...
public class Goat extends Animal{
    ...
    public void reproduce() {
        ...
        Goat partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Goat findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Goat && animal != this && animal.isAlive()) {
                            Goat otherGoat = (Goat) animal;

                            if (otherGoat.getAge() >= 3 && otherGoat.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherGoat.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Goat babyGoat = createBabyGoat();
            Launcher.addGoatToVisualMap(babyGoat);
            babyGoat.startLife();
            System.out.println("Родился новый олень: " + babyGoat.getName());
        }
    }

    private Goat createBabyGoat() {
        ...
        return new Goat(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
    ...
    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #008B8B; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}
