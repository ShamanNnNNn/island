...
public class Wolf extends Animal {
    ...
    public Wolf(String name, double weight, int age, Island island) {
    ...
    public Wolf(String name, Island island) {
        this(name,
    ...
    public boolean canEat(Object food) {
        if (food instanceof Animal) {
            ...
            return !(animal instanceof Wolf) && animal.isAlive();
        }
        return false;
    }
    ...
    public void reproduce() {
        Wolf partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Wolf findReproductionPartner() {
        ...
        for (int dx = -searchRadius; dx <= searchRadius; dx++) {
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Wolf && animal != this && animal.isAlive()) {
                            Wolf otherWolf = (Wolf) animal;

                            if (otherWolf.getAge() >= 2 && otherWolf.getHungerLevel() <= 70) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherWolf.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Wolf babyWolf = createBabyWolf();
            Launcher.addWolfToVisualMap(babyWolf);
            babyWolf.startLife();
            System.out.println("Родился новый волк: " + babyWolf.getName());
        }
    }

    private Wolf createBabyWolf() {
        String[] names = {"Серый", "Быстрый", "Сильный", "Вожак", "Охотник", "Лунный", "Тень", "Храбрый"};
        String babyName = names[random.nextInt(names.length)] + " от " + name;

        return new Wolf(babyName, 10.0 + random.nextDouble() * 5, 0, island);
    }
    ...
    public static Wolf createRandomWolf(Island island) {
        String[] names = {"Серый", "Быстрый", "Сильный", "Ауфыч", "Охотник", "Лунный", "Тень", "Храбрый"};
        String name = names[new Random().nextInt(names.length)];
        return new Wolf(name, island);
    }

    @Override
    public String toString() {
        return String.format("Волк '%s' (%d лет, %.1f кг) [голод: %d/100] в %s",
