...
public class Rabbit extends Animal{
    private static final String TYPE = "Кролик";
    private static final double MAX_WEIGHT = 10.0;
    private static final double MIN_WEIGHT = 3.0;
    private static final double MAX_FOOD_REQUIRED = 3.0;
    ...
    public Rabbit(String name, double weight, int age, Island island) {
        super(name, TYPE, weight, 20, MAX_FOOD_REQUIRED);
        this.hungerLevel = 30;
        this.age = age;
        this.island = island;
    ...
    public void reproduce() {
        ...
        Rabbit partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Rabbit findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Rabbit && animal != this && animal.isAlive()) {
                            Rabbit otherRabbit = (Rabbit) animal;

                            if (otherRabbit.getAge() >= 3 && otherRabbit.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherRabbit.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Rabbit babyRabbit = createBabyRabbit();
            Launcher.addRabbitToVisualMap(babyRabbit);
            babyRabbit.startLife();
            System.out.println("Родился новый кролик: " + babyRabbit.getName());
        }
    }

    private Rabbit createBabyRabbit() {
        String[] names = {"Белый", "Мелкий", "Ушастый"};
        String babyName = names[random.nextInt(names.length)] + " от " + name;

        return new Rabbit(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
    ...
    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #D8BFD8; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}
