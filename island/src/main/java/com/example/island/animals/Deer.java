...
public class Deer extends Animal {
    ...
    protected boolean tryMoveRandomly() {
        ...
        int maxAttempts = 5; // Увеличим количество попыток для надежности

        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            // Упростим логику перемещения - всегда двигаемся на 1 клетку в случайном направлении
            int deltaX = random.nextInt(3) - 1; // -1, 0, или 1
            int deltaY = random.nextInt(3) - 1; // -1, 0, или 1

            // Исключаем случай когда оба смещения равны 0 (остаемся на месте)
            ...
            if (newX >= 0 && newX < island.getSize() && newY >= 0 && newY < island.getSize()) {
                ...
                if (newLocation != null && move(newLocation)) {
                    ...
                    hungerLevel += 1 + distance; // Уменьшил затраты голода на перемещение
        ...
        hungerLevel += 1; // Уменьшил штраф за неудачную попытку перемещения
    ...
    public void reproduce() {
        ...
        Deer partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Deer findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Deer && animal != this && animal.isAlive()) {
                            Deer otherDeer = (Deer) animal;

                            if (otherDeer.getAge() >= 3 && otherDeer.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherDeer.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Deer babyDeer = createBabyDeer();
            Launcher.addDeerToVisualMap(babyDeer);
            babyDeer.startLife();
            System.out.println("Родился новый олень: " + babyDeer.getName());
        }
    }

    private Deer createBabyDeer() {
        String[] names = {"Бурый", "Рогач", "Стремительный", "Лесной", "Горный", "Северный", "Благородный", "Белохвост"};
        String babyName = names[random.nextInt(names.length)] + " от " + name;

        return new Deer(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
    ...
    private void setupVisualAppearance() {
        this.getVisualRepresentation().setStyle("-fx-fill: #F4A460; -fx-stroke: black; -fx-stroke-width: 1;");
    }
}