...
public class Mause extends Animal{
    ...
    public void reproduce() {
        ...
        Mause partner = findReproductionPartner();
        if (partner == null) {
            return;
        }
    ...
    private Mause findReproductionPartner() {
            ...
            for (int dy = -searchRadius; dy <= searchRadius; dy++) {
                ...
                if (searchX >= 0 && searchX < island.getSize() &&
                    ...
                    for (Animal animal : animalsInLocation) {
                        if (animal instanceof Mause && animal != this && animal.isAlive()) {
                            Mause otherMause = (Mause) animal;

                            if (otherMause.getAge() >= 3 && otherMause.getHungerLevel() <= 60) {
                                System.out.println(name + " нашел партнера для размножения: " +
                                        otherMause.getName() + " на расстоянии " +
    ...
    private void createChildren(int childrenCount) {
        for (int i = 0; i < childrenCount; i++) {
            Mause babyMause = createBabyMause();
            Launcher.addMauseToVisualMap(babyMause);            babyMause.startLife();
            System.out.println("Родился новый олень: " + babyMause.getName());
        }
    }

    private Mause createBabyMause() {
        ...
        return new Mause(babyName, 30.0 + random.nextDouble() * 20, 0, island);
    }
